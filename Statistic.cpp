...
void Statistic::add(double value)
{
	...
	if( value > _decay_max ){
		_decay_max = value;
	} else if (_decay_max > average()) {
		_decay_max -= (_decay_max-average()) * _decay;
	}
	
	if( value < _decay_min ){
		_decay_min = value;
	} else if (_decay_min < average()) {
		_decay_min += (average()-_decay_min) * _decay;
	}
#endif
}
...
#ifdef STAT_USE_DECAY
double Statistic::decay_maximum(){
	return _decay_max;
}

double Statistic::decay_minimum(){
	return _decay_min;
}

double Statistic::decay_fraction(double value){
	return (value - _decay_min)/(_decay_max - _decay_min);
}

int Statistic::decay_fraction_int(double value){
	return (int)(255*decay_fraction(value));
}

void Statistic::set_decay(float decay){
	_decay = decay;
}
#endif
